...
def findPreferredProvider(pn, cfgData, dataCache, pkg_pn = None, item = None):
    ...
    if required_v is not None:
        if preferred_v is not None:
            logger.warn("REQUIRED_VERSION and PREFERRED_VERSION for package %s are both set using REQUIRED_VERSION %s", pn, required_v)
        else:
            logger.debug("REQUIRED_VERSION is set for package %s", pn)
